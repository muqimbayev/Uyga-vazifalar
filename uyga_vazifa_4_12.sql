...
join locations on locations.location_id = departments.LOCATION_ID where locations.country_id='US');



-- 4. Write a SQL subquery to find the first_name and last_name of the employees who are working as a manager
Select first_name, last_name from employees
where JOB_ID in (Select job_id from employees where job_id like '%MAN');
...
Select first_name, last_name, salary from employees where (job_id, salary) in (Select job_id, min(salary) from employees group by job_id); 
...
Select first_name, last_name, salary from employees 
where salary > (Select salary from employees where last_name='Bull') order by first_name;
